# `@e2ee-lib/e2ee-auth` Package Guide

This page documents the public `@e2ee-lib/e2ee-auth` API that a consumer must use
...
| --- | --- | --- |
| `@e2ee-lib/e2ee-auth/web` | browser-based apps such as Next.js | includes the full web feature set, including API-token helpers and multi-recipient encryption helpers |
| `@e2ee-lib/e2ee-auth/native` | React Native / Expo apps | omits the web-only token helpers and the multi-recipient helper |

The root export `@e2ee-lib/e2ee-auth` currently resolves to the web entry point.
...
rectangle "@e2ee-lib/e2ee-auth" as Package {
...
`@e2ee-lib/e2ee-auth` is responsible for:
...
- choosing the correct import path: `@e2ee-lib/e2ee-auth/web` or `@e2ee-lib/e2ee-auth/native`
...
use `@e2ee-lib/e2ee-auth/web`. If your app only needs password-based mobile flows,
`@e2ee-lib/e2ee-auth/native` is the matching surface.
